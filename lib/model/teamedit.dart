...
Future<List<S2Choice<String>>> buildSelectListValue() async {
  List<S2Choice<String>> _list = List<S2Choice<String>>();
  TeamDao _dao = TeamDao();
  List<TeamDto> _dto = await _dao.select(TableUtil.cId);

  for (int i = 0; i < _dto.length; i++) {
    _list.add(S2Choice<String>(value: _dto[i].id.toString(), title: _dto[i].name));
  }

  return _list;
}

Future confirmTeamValue(ApplicationBloc _bloc, TeamDto _selected, List<FormDto> _form) async {
  TeamDao _dao = TeamDao();
